...
void UUPMGraphicsManager::SetResolutionScale(float Percentage)
{
	...
	if (Settings)
	{
		Settings->SetResolutionScaleValueEx(FMath::Clamp(Percentage, 0.0f, 100.0f));
		MarkPendingChanges();
	}
}
...
{
	UGameUserSettings* Settings = GetGameUserSettings();
	return Settings ? Settings->GetResolutionScaleInformationEx().CurrentScaleValue : 100.0f;
}
