...
public:
	...
	/**
	 * Check if there are unsaved extended settings changes
	 * Returns true if any extended setting has been modified since last save
	 */
	UFUNCTION(BlueprintPure, Category = "UPM|Extended")
	bool HasUnsavedExtendedChanges() const { return bHasUnsavedChanges; }
...
protected:
	...
	/** Mark that extended settings have been changed */
	void MarkExtendedSettingsChanged();

	/** Tracks unsaved changes for extended settings */
	bool bHasUnsavedChanges = false;

	/** Friend class for save/load to clear unsaved flag */
	friend class UUPMSaveLoad;
